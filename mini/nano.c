...
// usage, bus, hard,test 2 can be mpu fault, test 1, allocate array on board [free 1536 allocate 1024 and it should put it in there]
    ...
    else if (isCommand(&data, "hard", 0))
    {

        causehard();
    }
    else if (isCommand(&data, "usage", 0))
    {
        causeusage();
    }
    else if (isCommand(&data, "bus", 0))
    {
        causebus();
    }
...
int main()
{
    ...
    //setmsp((uint32_t*) 0x20001000);
    initHW();
    initUart0();
    setUart0BaudRate(115200,40000000U);


    putsUart0("Clock=40 MHz, UART0=115200 8N1\n");

    // Enable fault handlers and traps
    enableFaultsAndTraps();
    allowFlashAccess();
    allowPeripheralAccess();
    setupSramAccess();
    srdMasks = createNoSramAccessMask();
    addSramAccessWindow(&srdMasks, (uint32_t*)0x20001000, 28672);  // FIXED: 28672
    applySramAccessMask(srdMasks);
    enablempu();
    clearunpriv(); //sets to privilege


    //msp @ 0x20001000 going upwards
    //psp @ 0x20008000 going downwards
